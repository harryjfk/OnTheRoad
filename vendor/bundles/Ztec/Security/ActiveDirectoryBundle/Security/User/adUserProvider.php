...
class adUserProvider implements UserProviderInterface
{
    ...
    public function fetchData(adUser $adUser, adLDAP $adLdap){
        ...
        if ($user) {
            ...
            $usuario = $em->getRepository('PublicoBundle:Usuario')->findOneBy(
				array('usuario' => $adUser->getUsername())
            );
			...
			$sfRoles[] = 'ROLE_'.strtoupper(str_replace(' ','_',$usuario->getRol()->getNombre()));
